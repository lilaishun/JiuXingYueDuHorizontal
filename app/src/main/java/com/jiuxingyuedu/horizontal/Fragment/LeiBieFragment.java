...
public class LeiBieFragment extends LazyFragment implements View.OnClickListener {
    ...
    List<NewsNean> CurrentList = new ArrayList<>();
    List<NewsNean> NewsNeanRiBao = new ArrayList<>();
    List<NewsNean> NewsNeanChenBao = new ArrayList<>();
    List<NewsNean> NewsNeanJingji = new ArrayList<>();
    List<NewsNean> NewsNeanFazhi = new ArrayList<>();
    List<NewsNean> NewsNeanQita = new ArrayList<>();
    List<NewsNean> NewsNeanLeibei = new ArrayList<>();
    ...
    @BindView(R.id.chenbao)
    LinearLayout chenbao;
    @BindView(R.id.ribao)
    LinearLayout ribao;
    @BindView(R.id.wanbao)
    LinearLayout wanbao;
    @BindView(R.id.dushibao)
    LinearLayout dushibao;
    @BindView(R.id.jingjibao)
    LinearLayout jingjibao;
    @BindView(R.id.shenghuobao)
    LinearLayout shenghuobao;
    @BindView(R.id.fazhibao)
    LinearLayout fazhibao;
    @BindView(R.id.jiaoyubao)
    LinearLayout nongyebao;
    @BindView(R.id.junshibao)
    LinearLayout junshibao;
    @BindView(R.id.tiyubao)
    LinearLayout tiyubao;
    @BindView(R.id.qingshaonianbao)
    LinearLayout qingshaonianbao;
    @BindView(R.id.baojianbao)
    LinearLayout baojianbao;
    @BindView(R.id.qitabao)
    LinearLayout qitabao;
...
private ArrayList<NewsNean> NewsList=new ArrayList();
    ...
    public static LeiBieFragment newInstance(int tabIndex, boolean isLazyLoad) {

        Bundle args = new Bundle();
        args.putInt(INTENT_INT_INDEX, tabIndex);
        args.putBoolean(LazyFragment.INTENT_BOOLEAN_LAZYLOAD, isLazyLoad);
        LeiBieFragment fragment = new LeiBieFragment();
    ...
    protected void onCreateViewLazy(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.fragment_leibie_item);
...
ButterKnife.bind(this,getLayContentView());
       ...
       ribao.setOnClickListener(this);

       wanbao.setOnClickListener(this);

       dushibao.setOnClickListener(this);

      jingjibao.setOnClickListener(this);

       shenghuobao.setOnClickListener(this);

       fazhibao.setOnClickListener(this);

       nongyebao.setOnClickListener(this);

       junshibao.setOnClickListener(this);

     tiyubao.setOnClickListener(this);

   qingshaonianbao.setOnClickListener(this);

      baojianbao.setOnClickListener(this);

     qitabao.setOnClickListener(this);
        ...
        mActivity.et_seacher.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(!mActivity.ISShowSeach){
                    ...
                    setStateFr(CurrentList,0);
                    setSeacher("", CurrentList);
                    ISSHOWSEACHER=true;
                }else{
                    ISSHOWSEACHER=false;
        ...
        mActivity.search_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(!"".equals(mActivity.et_seacher.getText().toString().trim())) {
                    ...
                    mActivity.ll_earch1.setVisibility(View.VISIBLE);
                    setSeach(CurrentList, mActivity.et_seacher.getText().toString().trim());
...
//if(!"".equals(s.toString())) {
                if(ISSHOWSEACHER){
                    ...
                    setSeacher(s.toString(), CurrentList);
                }
       ...
       // Selection.setSelection((Spannable)s, s.toString().length());
        ...
        mActivity.hlv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                System.out.println("name=============+leibie===="+objects.size());
...
DesTimer();
    ...
    public void ClearData(){
        ...
        currentIndex=0;
        if(null!=lei_ll){
            lei_ll.setVisibility(View.VISIBLE);
        }
    if(null!=CurrentList){
        CurrentList.clear();
    }
            ...
            switch (msg.what) {
                case 1:
                    for (int i = 0; i <NewsNeanList.size() ; i++) {
                        ...
                        if("日报".equals(newsNean.getCategory())){
NewsNeanRiBao.add(newsNean);
                        }else if("晨报".equals(newsNean.getCategory())){
                            NewsNeanChenBao.add(newsNean);
                        }else if("经济报".equals(newsNean.getCategory())){
                            NewsNeanJingji.add(newsNean);
                        }
                        else if("法制报".equals(newsNean.getCategory())){
                            NewsNeanFazhi.add(newsNean);
                        }
                        else{
                            NewsNeanQita.add(newsNean);
                        }

                    }
                    NewsNeanList.clear();;
                    NewsNeanList.addAll(NewsNeanRiBao);
                    NewsNeanList.addAll(NewsNeanChenBao);
                    NewsNeanList.addAll(NewsNeanJingji);
                    NewsNeanList.addAll(NewsNeanFazhi);
                    NewsNeanList.addAll(NewsNeanQita);
 ...
 */
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()){
            ...
            case R.id.tiyubao:
                setfragmentData("体育报");
                break;
            case R.id.qingshaonianbao:
                setfragmentData("青少年报");
                break;
            case R.id.baojianbao:
                setfragmentData("保健报");
                break;
            case R.id.qitabao:
                setfragmentData("其他报");
    ...
    private void setfragmentData(String name){
        lei_ll.setVisibility(View.GONE);
        CurrentList.clear();
        for (int i = 0; i <NewsNeanList.size() ; i++) {
            NewsNean newsNean = NewsNeanList.get(i);
            if(name.equals(newsNean.getCategory())){
                CurrentList.add(newsNean);
            }
        }
